  ...
  adb.findDevices()
    .then(function(devices) {
      ...
      inquirer.prompt(choices, function(answers){
        ...
        adb.pullFile(deviceId, USERJS).then(function(userjs){
          ...
          readable.push(null);
          adb.pushFile(deviceId, USERJS, readable).then(function() {
            console.log(chalk.green(
                          '[%s] Successfully pushed proxy conf to %s'),
                        deviceId, USERJS);
  ...
  return new Promise(function(resolve, reject) {
    adb.pullFile(deviceId, devicePath, targetPath)
      .then(function() {
        ...
        var backupPath = util.format('%s.%s', targetPath, deviceId);
...
function addCerts() {
  ...
  console.log(chalk.cyan('Certutil installed at: %s'), certutilPath);
  adb.findDevices()
    .then(function(devices) {
      var deviceList = adb.getDevicesList(devices);
      ...
      function handleAnswers(answers) {
        ...
        adb.getProfilePaths(deviceId)
          .then(function(profilePath) {
            ...
            var certTargetPath = path.join(__dirname + '/../tmp/cert9.db');
            var keyTargetPath = path.join(__dirname + '/../tmp/key4.db');
