...
function addCerts() {
  ...
  return adb.findDevices()
    .then(function(devices) {
            ...
            return Promise.all(promises)
              .then(function() {
                ...
                return clearCertPassword()
                  .then(function() {
                    return addCertToDb(answers.cert)
                      .then(function() {
                        return checkCertIsValid()
                          .then(function() {
                            log.success('[certs] Cert is valid o/');
                            var cert = fs.createReadStream(certTargetPath);
                            var key = fs.createReadStream(keyTargetPath);
                            var promises = [
                              adb.pushFile(deviceId, certDevicePath, cert),
                              adb.pushFile(deviceId, keyDevicePath, key)
                            ];
                            return Promise.all(promises).then(function() {
                              log.success('[%s] Successfully pushed certs',
                                          deviceId);
                              return adb.rebootDevice(deviceId);
                            });
                          });
      ...
      choices.push(questions.chooseCert(settings.certDir));
      inquirer.prompt(choices, handleAnswers);
    }).catch(utils.handleError);
}
